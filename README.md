...
**场景 4：中断命令（明白了，直接记录）**
```
你：这句话什么意思？"The ephemeral beauty..."
AI：这句话的难点在于 ephemeral。
    Ephemeral (adj) means lasting for a very short time...
    
你：/ok  ← 表示"明白了，不用再问了"
AI：✅ 已记录到今日学习笔记 (2025-12-31.md)
    
    [立即停止提问，直接记录，不会再问验证问题]
```
...
| 命令 | 功能 | 说明 |
|------|------|------|
| `/ok` | 中断并记录 | 对话中发送，表示"明白了，别再问了，直接记录" |
...
**正常模式（默认）**：完整的苏格拉底式教学
```
你提问 → AI 讲解 → AI 问"你能试着翻译/造句吗？" → 你回答 → AI 反馈 → 记录
```
- ✅ 适合深度学习，确保真正理解
- ✅ 通过互动验证掌握情况
- ⏱️ 时间较长，需要多轮对话
